...
const statusConfig: Record<string, { label: string; color: string; icon: string; step: number }> = {
  PENDING: { label: 'در انتظار پرداخت', color: 'bg-yellow-100 text-yellow-700 border-yellow-300', icon: '⏳', step: 0 },
  PAID: { label: 'پرداخت شده', color: 'bg-blue-100 text-blue-700 border-blue-300', icon: '💳', step: 1 },
  PROCESSING: { label: 'در حال پردازش', color: 'bg-purple-100 text-purple-700 border-purple-300', icon: '⚙️', step: 2 },
  SHIPPED: { label: 'ارسال شده', color: 'bg-indigo-100 text-indigo-700 border-indigo-300', icon: '🚚', step: 3 },
  DELIVERED: { label: 'تحویل شده', color: 'bg-green-100 text-green-700 border-green-300', icon: '✅', step: 4 },
  CANCELLED: { label: 'لغو شده', color: 'bg-red-100 text-red-700 border-red-300', icon: '❌', step: -1 },
  REFUNDED: { label: 'بازپرداخت شده', color: 'bg-gray-100 text-gray-700 border-gray-300', icon: '↩️', step: -1 },
};
              ...
              return (
                <div key={entry.id} className="flex gap-4 pb-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${idx === 0 ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                      {config?.icon || '•'}
                    </div>
